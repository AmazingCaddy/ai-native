...
export function renderMarkdownReport(records: ModelRunRecord[]): string {
  ...
  const runs = records.map(
    (record) =>
      `| ${cell(record.taskTitle)} | ${cell(record.model)} | ${record.repetition} | ${record.status} | ${record.elapsedMs} | ${cell(record.usage?.inputTokens)} | ${cell(record.usage?.outputTokens)} | 待评分 | 待评分 | 待评分 |`,
  );
